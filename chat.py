...
'''
Был выбран вариант №6 с использованием библиотеки click.
python chat.py get # слушать сообщения из очереди и выводить в чат
python chat.py send # отправить сообщение в чат (спрашивает имя пользователя и сообщение для отправки)
python chat.py --help # выводит список команд и параметров

Если не указано имя или сообщение, то оно спрашивается у пользователя. 
Если пользователь указал пустое имя пользователя то отправляется сообщение от имени Anonymous.
Если пользователь указал пустое сообщение, то отправляется сообщение Test
'''
